...
class OAuthTokenManager:
    ...
    async def authenticate(self) -> None:
        ...
        if not ready_event.is_set():
            raise TimeoutError("Timed out waiting for Keycloak authentication callback (120s)")

        if code_holder.get("error"):
            raise RuntimeError(f"Authorization failed: {code_holder['error']}")

        if code_holder.get("state") != state:
            raise RuntimeError("State mismatch in OAuth callback — possible CSRF attack")
    ...
    def _store_tokens(self, tokens: dict) -> None:
        ...
        # Keycloak may omit refresh_token on refresh responses in some configs — keep the old one then
